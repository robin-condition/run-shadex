...
impl InputSet {
    pub fn iter_inds<'a>(&'a self) -> InputSetIterator<'a> {
        let kvs: Vec<(&str, usize)> = self
            .params
            .iter()
            .map(|(a, b)| (a.as_ref(), b.cardinality() as usize))
            .collect();
        ...
        InputSetIterator {
            order: a,
            lens: b,
            cur: Some(MultiValueIndex {
                param_inds: starting_map,
            }),
            first: true,
        }
    }
...
pub trait MultiValueRepresentationTrait<El: SingleValueRepresentation>:
    AtLeastOneValueRepresentation<El>
{
}
...
impl<El: SingleValueRepresentation> AtLeastOneValueRepresentation<El>
    for GeneralPurposeMultiValueRepresentation<El>
{
    fn get<'a>(&self, ind: MultiValueIndex<'a>) -> El {
        match self {
...
impl<El: SingleValueRepresentation> MultiValueRepresentationTrait<El>
    for GeneralPurposeMultiValueRepresentation<El>
{
}
...
impl<El: PrimitiveValue> AtLeastOneValueRepresentation<El> for PrimitiveMultivalue<El> {
    fn get<'a>(&self, ind: MultiValueIndex<'a>) -> El {
        match self {
            PrimitiveMultivalue::General(general_purpose_multi_value_representation) => {
                general_purpose_multi_value_representation.get(ind)
            }
...
impl<El: SingleValueRepresentation> AtLeastOneValueRepresentation<El>
    for NOrderValueRepresentation<El>
{
    fn get<'a>(&self, ind: MultiValueIndex<'a>) -> El {
        match self {
...
impl<El: SingleValueRepresentation> AtLeastOneValueRepresentation<El> for ValueArray<El> {
    fn get<'a>(&self, ind: MultiValueIndex<'a>) -> El {
        self.els[*ind
            .param_inds
            .get(AsRef::<str>::as_ref(&self.arg_name))
            .unwrap()]
        .get(ind)
    }
}
